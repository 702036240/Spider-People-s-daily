...
def getPageList(year, month, day):
    ...
    参数：年，月，日
    '''
    url = 'http://paper.people.com.cn/rmrbhwb/html/' + year + '-' + month + '/' + day + '/node_865.htm'
    html = fetchUrl(url)
    ...
    for page in pageList:
        if flag == 1:
            ...
            link = page.a['href']
        url = 'http://paper.people.com.cn/rmrbhwb/html/' + year + '-' + month + '/' + day + '/' + link
        linkList.append(url)
        flag+=1
...
def download_rmrb(year, month, day):
    ...
    for page in pageList:
        ...
        for url in titleList:
            ...
            #重新构建保存文件的名称
            #temp = url.split('_')[1].split('.')
            titleNo += 1
            path = 'E:/data(海外版)' + '/' + year + month + day + '/'
            if pageNo < 10:
                pageNo = '0'+str(pageNo)
            else:
                pageNo = str(pageNo)
            if titleNo < 10:
                titleNo = '0'+str(titleNo)
            else:
                titleNo = str(titleNo)
...
if __name__ == '__main__':
    ...
    for d in data:
        ...
        time.sleep(3)        # 怕被封 IP 爬一爬缓一缓，爬的少的话可以注释20190703掉